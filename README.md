...
useEffect(() => {
  fetch('[https://your-username--credit-api.modal.run](https://your-username--credit-api.modal.run)')
    .then(res => res.json())
    .then(response => {
...
def analyze(user_data, current_score):
    ...
    response = requests.post("[https://api.tokenbus.io/](https://api.tokenbus.io/)...", json={"prompt": prompt})
